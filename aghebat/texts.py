...
# پیام تبلیغاتی شروع برای کاربران فعال‌نشده (فقط چت خصوصی؛ قابل تنظیم با /settext نیست).
# متغیر {owner} توسط render_start_promo با روش تماس مالک جایگزین می‌شود.
START_PROMO = (
    "🔒 <b>عاقبت برای تو هنوز فعال نشده!</b>\n\n"
    "این بات ویژه‌ست و فقط برای کاربران تاییدشده کار می‌کنه.\n"
    "کافیه با مالک بات تماس بگیری تا فعال‌سازی کنه 🤝\n\n"
    "✨ <b>عاقبت چیکار می‌کنه؟</b>\n"
    "🎲 هر روز برای اعضای گروه یه «عاقبت» تصادفی رقم می‌زنه؛ گاهی سود، گاهی زیان!\n"
    "🏆 با امتیاز چت و نردبان رتبه، گروهت رو رقابتی و پرانرژی می‌کنه.\n"
    "🔓 بالا رفتن رتبه، دسترسی‌های جدید گروه رو خودکار باز می‌کنه.\n\n"
    "{owner}"
)


def render_start_promo(owner_contact: str = "") -> str:
    """پیام تبلیغاتی شروع را با جمله تماس با مالک کامل می‌کند (جایگزینی دستی)."""
    if owner_contact:
        owner_line = f"📞 <b>فعال‌سازی:</b> با مالک بات در تماس باش: {html.escape(owner_contact)}"
    else:
        owner_line = "📞 <b>فعال‌سازی:</b> با مالک بات تماس بگیر تا اسمت رو ثبت کنه."
    return START_PROMO.replace("{owner}", owner_line)
